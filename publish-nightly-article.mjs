...
// Direct URL upload — for LOC, NARA, IWM, Bundesarchiv, etc.
async function tryUploadDirectUrl(imageUrl, slug) {
  const ext = imageUrl.split('.').pop().split('?')[0].toLowerCase()
  const mimeType = mime(ext)
  const referer = new URL(imageUrl).origin + '/'
  const head = await fetch(imageUrl, { method: 'HEAD', headers: { 'User-Agent': UA, Referer: referer } })
  const cl = parseInt(head.headers.get('content-length') || '0', 10)
  if (cl > MAX) throw new Error(`Too large: ${(cl / 1024).toFixed(0)}KB`)
  const img = await fetch(imageUrl, { headers: { 'User-Agent': UA, Referer: referer } })
  if (!img.ok) throw new Error(`Download failed: ${img.status}`)
  const buf = await img.arrayBuffer()
  if (buf.byteLength > MAX) throw new Error('Buffer too large after download')
  const r = await fetch(`${ASSETS}?filename=${encodeURIComponent(slug)}`, {
...
async function tryUploadWithFallback(imageFile, slug) {
  // Direct URL (LOC, NARA, IWM, Bundesarchiv, etc.)
  if (imageFile.startsWith('https://')) {
    try { return await tryUploadDirectUrl(imageFile, slug) } catch (e) {
      console.log(`  ✗ Direct URL upload failed: ${e.message}`)
      return null
    }
  }
...
// Look up an article's _id by its slug — needed to build Sanity references.
// Returns null if the article doesn't exist yet (graceful fallback).
async function lookupArticleId(slug) {
  const q = encodeURIComponent(
    `*[_type == "article" && slug.current == "${slug}" && !(_id in path("drafts.**"))][0]._id`
  )
  try {
    const r = await fetch(`${BASE}/query/${DATASET}?query=${q}`, {
...
// Publish a single onThisDayEntry document.
// Uses createOrReplace with a deterministic _id so re-running is idempotent.
// Expected shape of `otd`:
//   { month, day, year, headline, summary, era, linkedArticleSlug? }
async function publishOnThisDay(otd) {
  const mm  = String(otd.month).padStart(2, '0')
  const dd  = String(otd.day).padStart(2, '0')
  const doc = {
    _id:      `otd-${otd.year}-${mm}-${dd}`,
    _type:    'onThisDayEntry',
    month:    otd.month,
    day:      otd.day,
    year:     otd.year,
    headline: otd.headline,
    summary:  otd.summary,
    era:      otd.era,
  }

  if (otd.linkedArticleSlug) {
    const articleId = await lookupArticleId(otd.linkedArticleSlug)
    if (articleId) {
      doc.linkedArticle = { _type: 'reference', _ref: articleId }
    } else {
      console.log(`    ↳ linkedArticleSlug "${otd.linkedArticleSlug}" not found — skipping link`)
    }
  }
...
// ── On This Day entry (optional — generated by the AI writer alongside the article) ──
if (data.onThisDay) {
  const otd = data.onThisDay
  const label = `${otd.year}-${String(otd.month).padStart(2,'0')}-${String(otd.day).padStart(2,'0')}`
  process.stdout.write(`\n  ▸ [OTD ${label}] ${otd.headline}\n`)
  try {
    await publishOnThisDay(otd)
    console.log(`    ✓ Published`)
    results.ok.push(`otd-${label}`)
  } catch (e) {
    console.log(`    ✗ ${e.message}`)
    results.fail.push(`otd-${label}`)
  }
} else {
  console.log(`\n  ↷ No onThisDay entry in data — skipping`)
}
