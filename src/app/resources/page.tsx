...
type ResourceItem = { name: string; url: string; description: string }
type ResourceSection = { title: string; items: ResourceItem[] }
type ResourcesData = {
  pageTitle?: string
  subtitle?: string
  footerNote?: string
...
// ─── Fallback data ───────────────────────────────────────────────────────────
const FALLBACK_SECTIONS: ResourceSection[] = [
  {
  ...
  return (
    <section className="mb-16">
      <div className="flex items-center gap-6 mb-8">
        ...
        <h2 className="font-body text-[0.6rem] tracking-[0.3em] uppercase text-mist">{section.title}</h2>
        <div className="flex-1 border-t border-rule" />
      </div>
...
export default async function ResourcesPage() {
  const data: ResourcesData | null = await client
    .fetch(resourcesPageQuery)
    .catch(() => null)

  const pageTitle  = data?.pageTitle  ?? 'Research Resources'
  const subtitle   = data?.subtitle   ?? 'Primary source archives, official photograph collections, and research repositories used in compiling Library of War. All sources are publicly accessible.'
  const footerNote = data?.footerNote ?? 'Library of War does not endorse any specific viewpoint represented in external archives. All linked repositories are official institutional or government sources. Content availability may change over time as archives update their digital collections.'
