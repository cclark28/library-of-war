...
const ERA_TABS = [
  { label: 'All',               era: null },
  { label: 'WWI',               era: 'world-war-i' },
  { label: 'WWII',              era: 'world-war-ii' },
  { label: 'Korean War',        era: 'korean-war' },
  { label: 'Vietnam',           era: 'vietnam-war' },
  { label: 'Cold War',          era: 'cold-war' },
  { label: 'Modern',            era: 'modern-conflicts' },
  { label: 'Ancient & Medieval', era: 'ancient-medieval' },
...
function matchesEra(article: Article, era: string): boolean {
  if (article.era === era) return true
  if (article.categories?.some(c => c.slug.current === era)) return true
  return false
}

/* ── Component ───────────────────────────────────────────────────────────── */

export default function BrowseClient({ allArticles, initialEra }: Props) {
  const [activeEra, setActiveEra]   = useState<string | null>(initialEra)
  const [gridVisible, setGridVisible] = useState(true)
  const fadeTimeout                 = useRef<ReturnType<typeof setTimeout> | null>(null)

  // Filtered list derived from activeEra
  const filtered = activeEra
    ? allArticles.filter(a => matchesEra(a, activeEra))
    : allArticles

  const switchEra = useCallback((era: string | null) => {
    ...
    fadeTimeout.current = setTimeout(() => {
      ...
      const url = era ? `/browse?era=${era}` : '/browse'
  ...
  // Sync if browser back/forward navigation changes the URL
  useEffect(() => {
    const onPopState = () => {
      const params = new URLSearchParams(window.location.search)
      setActiveEra(params.get('era'))
      setGridVisible(true)
    }
  ...
  const meta      = activeEra ? ERA_META[activeEra] : null
    ...
    <>
      ...
      <div className="border-b border-rule bg-ghost overflow-x-auto">
        <div className="max-w-7xl mx-auto px-6 py-0 flex items-stretch gap-0">
          {ERA_TABS.map((tab) => {
            const isActive = tab.era === activeEra
            return (
              <button
                key={tab.era ?? 'all'}
                onClick={() => switchEra(tab.era)}
                className={[
          ...
          {filtered.length > 0 ? (
            <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-0 border border-rule mb-8">
                ...
                <div className="lg:col-span-3 border-b lg:border-b-0 lg:border-r border-rule p-6 md:p-8">
                  <ArticleCard article={filtered[0]} size="lg" showExcerpt />
                </div>
            ...
            <div className="text-center py-28 border border-rule">
              ...
              <p className="font-body text-mist/50 text-sm">
                {activeEra ? `The ${pageTitle} archive is being compiled.` : 'The archive is being assembled.'}
              </p>
            </div>
          )}
        </div>
